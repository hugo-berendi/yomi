attrs @ {
  ...
  types = {
    ...
    # }}}
    # {{{ Lazy module
    lazyModule = lazyType "actually lazy lazy module" (_:
      struct "lazy module"
      {
        package = k.string;
    ...
    # }}}
    # {{{ Tempest config
    tempestConfig = lazyType "lazy tempest config" (_:
      struct "tempest config"
      {
        vim = types.luaValue;
  ...
  hasType = type: value: let
    err = type.verify value;
  in
    lib.assertMsg (err == null) err;

  mkLib = {tempestModule}:
    assert hasType k.string tempestModule; rec {
      inherit (e) encode;
      ...
      foldedList = value:
        assert hasType k.attrs value; {
          inherit value;
          __luaEncoderTag = "foldedList";
        };
      thunk = code: _: lua code;
      tempest = given: context:
        lua ''
          D.tempest.configure(
            ${encode given},
            ${context}
      ...
      tempestBufnr = given: context:
        ...
        '';
      keymap = mode: mapping: action: desc: {inherit mode mapping action desc;};
      ...
      generateConfig = rawConfig:
        assert hasType types.neovimConfig rawConfig; let
          config =
            {
              lazy = {};
              pre = {};
