...
in {
  ...
  # Needs the network for IMAP and for the classifier on loopback. The strict
  # tier already permits AF_INET/AF_INET6/AF_UNIX, so allowNetwork (which only
  # adds AF_NETLINK) is not wanted here.
  yomi.hardening.services.mail-sorter.tier = "strict";
}
